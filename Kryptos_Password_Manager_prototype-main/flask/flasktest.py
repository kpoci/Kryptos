...
def register():
    if request.method == 'POST':
        # Retrieve form data, including the new security password
        username = request.form['username']
        email = request.form['email']
        password = request.form['password']
        master_pass = request.form['master_pass']
        security_password = request.form['security_password']  # New line added

        # Hash the passwords
        hashed_password = ph.hash(password)
        hashed_master_pass = ph.hash(master_pass)
        hashed_security_password = ph.hash(security_password)  # New line added
        ...
        # Insert the new user into the database, including the security password
        cur.execute(
...
def verify_master_password():
    ...
    if stored_password and ph.verify(stored_password['master_pass'], master_password):
...
# Decrypt Password Route with Key Verification
